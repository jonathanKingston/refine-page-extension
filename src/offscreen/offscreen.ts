...
function makeInert(html: string, baseUrl?: string): string {
  ...
  doc.querySelectorAll('link').forEach((link) => {
    ...
    // Remove inline JS handlers on link elements early.
  ...
  // Using `disabled` can affect rendering on some sites (e.g., inputs can appear hidden
  // or lose critical styles). We instead prevent interaction via CSS (see inert styles
  // below) and a few attributes that don't generally alter appearance.
  ...
  // Remove ALL inline event handlers (on* attributes) for safety.

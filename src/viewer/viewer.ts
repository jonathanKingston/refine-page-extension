...
// Storage abstraction — works in both extension and standalone web mode.
...
async function persistSnapshot(snapshot: Snapshot): Promise<void> {
  ...
  // Standalone mode: update in-memory array
...
function updateUI() {
  ...
  if (iframe) {
    ...
    const iframeUrl = hasChromeStorage() ? chrome.runtime.getURL('iframe.html') : 'iframe.html';
...
// Show a modal to load a ZIP export when running in standalone web mode.
function showStandaloneZipLoader(): Promise<void> {
  return new Promise<void>((resolve) => {
    ...
    card.innerHTML = `
      ...
      <div style="font-size: 13px; color: rgba(249,250,251,0.75); margin-bottom: 12px;">
        Choose a <code>.zip</code> exported from the extension (Export button in the popup).
        This enables running eval fully in a normal web page (no extension APIs).
      </div>
